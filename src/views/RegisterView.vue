...
const handleSubmit = () => {
  ...
  axios
    ...
    .catch((err) => {
      ...
      if (error.firstname) {
        isprocessing.value = false
        message.value = error.firstname[0]
      } else if (error.lastname) {
        isprocessing.value = false
        message.value = error.lastname[0]
      } else if (error.email) {
        isprocessing.value = false
    ...
    >
      ...
      <div class="w-3/4 flex flex-col items-center">
        <article class="w-11/12 flex flex-col items-center">
          <section class="w-full text-left text-lg capitalize">firstname</section>
      ...
      <div class="w-3/4 flex flex-col items-center">
        <article class="w-11/12 flex flex-col items-center">
          <section class="w-full text-left text-lg capitalize">lastname</section>
          <article class="w-full">
            <input type="text" v-model="lastname" class="w-full p-2 rounded-md border" />
          </article>
        </article>
      </div>
